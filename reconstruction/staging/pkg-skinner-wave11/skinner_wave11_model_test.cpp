...
#define PKG_SKINNER_WAVE11_CDECL __cdecl
#else
#define PKG_SKINNER_WAVE11_THISCALL __attribute__((thiscall))
#define PKG_SKINNER_WAVE11_CDECL __attribute__((cdecl))
#endif
...
struct Trace {
  ...
  TargetWord acquire_object;
  TargetWord acquire_slot;
  TargetWord collect;
  OpaqueRefObject **collect_arg[8];
  TargetWord rewind;
  TargetWord release_one;
  TargetWord truncate;
  TargetWord lazy_reset;
  TargetWord job_init;
  TargetWord job_init_primary;
  TargetWord job_build;
  TargetWord ref_add;
  TargetWord ref_release;
  TargetWord ref_step;
  TargetWord manager_create_block;
  TargetWord manager_create_raw;
  TargetWord manager_can_create;
...
struct Fixture {
  ...
  OpaqueElementVTable element_vtable;
  ...
  bool can_create_answer = true;
  bool acquire_answer = true;
};

Fixture *g_fixture = nullptr;

extern "C" OpaquePaintSystem *PKG_SKINNER_WAVE11_CDECL
...
extern "C" void PKG_SKINNER_WAVE11_THISCALL stub_set_color_write_00529280(
...
extern "C" void PKG_SKINNER_WAVE11_THISCALL stub_emit_005295b0(
...
extern "C" TargetWord PKG_SKINNER_WAVE11_THISCALL
...
extern "C" void PKG_SKINNER_WAVE11_THISCALL
stub_copy_vec3_004098a0(OpaqueVec3f *out, const OpaqueVec3f *in) {
  out->a = in->a;
  out->b = in->b;
  out->c = in->c;
}

extern "C" TargetWord PKG_SKINNER_WAVE11_CDECL
stub_classify_004bb860(TargetWord) {
  ++g_fixture->trace.classify;
  return static_cast<TargetWord>(g_fixture->classify_value);
}

extern "C" bool PKG_SKINNER_WAVE11_CDECL
...
extern "C" const OpaqueKeyBlock *PKG_SKINNER_WAVE11_THISCALL
stub_get_desc_00446ff0(OpaqueRefObject *) {
  ++g_fixture->trace.get_desc;
  static OpaqueKeyBlock block = {0x11u, 0x22u, 0x33u};
  return &block;
}

extern "C" const OpaqueKeyBlock *PKG_SKINNER_WAVE11_CDECL
stub_get_default_desc_006bb640() {
  ++g_fixture->trace.get_default_desc;
  static OpaqueKeyBlock block = {0x44u, 0x55u, 0x66u};
  return &block;
}

extern "C" void *PKG_SKINNER_WAVE11_CDECL
stub_acquire_object_00f473a0(TargetWord size, TargetWord, TargetWord,
                             TargetWord, TargetWord, TargetWord) {
  ++g_fixture->trace.acquire_object;
  g_fixture->acquire_log[g_fixture->acquire_count] = size;
  ++g_fixture->acquire_count;
  if (!g_fixture->acquire_answer) {
    return nullptr;
  }
  return size >= kAcquireSizeWide ? &g_fixture->target2 : &g_fixture->target;
}

extern "C" OpaqueRefObject *PKG_SKINNER_WAVE11_THISCALL
...
extern "C" void PKG_SKINNER_WAVE11_THISCALL
stub_acquire_slot_00552750(OpaqueSlotHandle *handle) {
  ++g_fixture->trace.acquire_slot;
  handle->field_000 = reinterpret_cast<TargetWord>(g_fixture->slot_triple);
  handle->field_004 = 0x3333u;
}

extern "C" void PKG_SKINNER_WAVE11_THISCALL
stub_collect_0051d1e0(OpaqueVector *, OpaqueRefObject **argument) {
  if (g_fixture->trace.collect < 8u) {
    g_fixture->trace.collect_arg[g_fixture->trace.collect] = argument;
  }
  ++g_fixture->trace.collect;
}

extern "C" void PKG_SKINNER_WAVE11_THISCALL
stub_rewind_0051d140(OpaqueVector *) {
  ++g_fixture->trace.rewind;
}

extern "C" TargetWord PKG_SKINNER_WAVE11_THISCALL
stub_release_one_00402420(OpaqueRefObject *) {
  ++g_fixture->trace.release_one;
  return 1u;
}
...
extern "C" void PKG_SKINNER_WAVE11_THISCALL
stub_job_init_005173a0(OpaqueRefObject *) {
  ++g_fixture->trace.job_init;
}

extern "C" TargetWord PKG_SKINNER_WAVE11_THISCALL
stub_job_init_primary_00517310(OpaqueRefObject *, TargetWord, TargetByte) {
  ++g_fixture->trace.job_init_primary;
  return reinterpret_cast<TargetWord>(&g_fixture->target);
}

extern "C" OpaqueRefObject *PKG_SKINNER_WAVE11_THISCALL stub_job_build_0051c470(
    OpaqueRefObject *, TargetWord a0, TargetWord a1, TargetWord a2,
    TargetByte a3, TargetWord a4, TargetByte a5, TargetByte a6) {
  ++g_fixture->trace.job_build;
  BuildRecord &record = g_fixture->build[g_fixture->build_count];
  record.arg[0] = a0;
  record.arg[1] = a1;
  record.arg[2] = a2;
  record.arg[3] = a3;
  ...
  record.arg[5] = a5;
  record.arg[6] = a6;
  ++g_fixture->build_count;
  return &g_fixture->target2;
}

extern "C" OpaqueRefVTable *PKG_SKINNER_WAVE11_CDECL
...
stub_ref_add_000(OpaqueRefObject *object) {
  ++g_fixture->trace.ref_add;
  object->refcount_008 += 1u;
  return object->refcount_008;
}
...
stub_ref_step_010(OpaqueRefObject *) {
  ++g_fixture->trace.ref_step;
  return 0u;
}

extern "C" OpaqueRefObject *PKG_SKINNER_WAVE11_THISCALL stub_create_01c(
...
extern "C" OpaqueRefObject *PKG_SKINNER_WAVE11_THISCALL stub_create_020(
    OpaqueRenderTargetManager *, TargetWord, TargetWord, TargetWord key) {
  ++g_fixture->trace.manager_create_raw;
  CreateRecord &record = g_fixture->create[g_fixture->create_count];
  record.tag = key;
  ++g_fixture->create_count;
  return &g_fixture->target2;
...
extern "C" TargetWord PKG_SKINNER_WAVE11_THISCALL
stub_can_create_024(OpaqueRenderTargetManager *, OpaqueKeyBlock) {
  ++g_fixture->trace.manager_can_create;
  return g_fixture->can_create_answer ? 1u : 0u;
}
...
extern "C" TargetWord PKG_SKINNER_WAVE11_THISCALL stub_has_01c(OpaqueElement *,
                                                               TargetWord key) {
  ++g_fixture->trace.element_has;
  return (g_fixture->key_mask & (1 << (key & 3u))) != 0 ? 1u : 0u;
}

extern "C" OpaqueRefObject *PKG_SKINNER_WAVE11_THISCALL
stub_get_028(OpaqueElement *, TargetWord) {
  ++g_fixture->trace.element_get;
  return &g_fixture->target;
}
...
void install_ports() {
  ...
  ports.copy_vec3_004098a0 = stub_copy_vec3_004098a0;
  ...
  ports.get_desc_00446ff0 = stub_get_desc_00446ff0;
  ports.get_default_desc_006bb640 = stub_get_default_desc_006bb640;
  ports.acquire_object_00f473a0 = stub_acquire_object_00f473a0;
  ports.get_raster_00525aa0 = stub_get_raster_00525aa0;
  ports.acquire_slot_00552750 = stub_acquire_slot_00552750;
  ports.collect_0051d1e0 = stub_collect_0051d1e0;
  ports.rewind_0051d140 = stub_rewind_0051d140;
  ports.release_one_00402420 = stub_release_one_00402420;
  ports.truncate_004259e0 = stub_truncate_004259e0;
  ports.lazy_reset_0041d870 = stub_lazy_reset_0041d870;
  ports.job_init_005173a0 = stub_job_init_005173a0;
  ports.job_init_primary_00517310 = stub_job_init_primary_00517310;
  ports.job_build_0051c470 = stub_job_build_0051c470;
  ports.resolve_ref_vtable = stub_resolve_ref_vtable;
  g_skinner_wave11_ports = ports;
}
...
void setup(Fixture &f, TargetSignedWord rig_count) {
  ...
  f.can_create_answer = true;
  f.acquire_answer = true;
  ...
  f.manager_vtable.create_020 = stub_create_020;
  f.manager_vtable.can_create_024 = stub_can_create_024;
  f.manager_vtable.prepare_034 = stub_prepare_034;
  f.element_vtable.has_01c = stub_has_01c;
  f.element_vtable.get_028 = stub_get_028;
  ...
  f.owner[1] = 0x1000u;
  f.owner[2] = 0x1000u;
  f.slot_triple[0] = 0x5555u;
  f.slot_triple[1] = reinterpret_cast<TargetWord>(&f.owner[3]);
  f.slot_triple[2] = reinterpret_cast<TargetWord>(&f.owner[4]);
  f.slot_triple[3] = 0u;
  ...
  for (TargetSignedWord i = 0; i < 16; ++i) {
    ...
    f.mesh_id[i] = reinterpret_cast<TargetWord>(&f.element);
  }
  ...
  f.element.vtable_000 = &f.element_vtable;
  ...
  g_fixture = &f;
  install_ports();
  g_skinner_wave11_scale_source = nullptr;
  std::memset(g_skinner_wave11_scale, 0, sizeof(g_skinner_wave11_scale));
}
...
void test_rig_pass_state_runs_four_passes() {
  ++g_tests;
  Fixture f;
  setup(f, 2);
  OpaqueSkinJobState job{};

  check(skin_rig_pass_state_00519640(&job) == 0u, "rig pass zero returns zero");
  check(job.field_010 == 1, "rig pass advances pass index");
  check(job.field_014 == 0, "rig pass resets rig cursor on advance");
  check(f.paint_count == 2u, "rig pass zero paints every rig block");
  check(f.paint[0].a0[0] == 0.0F && f.paint[0].a1[0] == 1.0F,
        "rig pass reads the mesh slot entry");

  check(skin_rig_pass_state_00519640(&job) == 0u, "rig pass one returns zero");
  check(job.field_010 == 3, "rig pass skips the remapped slot one");
  check(f.paint_count == 4u, "rig pass one paints every rig block");

  check(skin_rig_pass_state_00519640(&job) == 1u, "rig pass four returns one");
  check(job.field_010 == 4, "rig pass reaches the terminal pass");
  check(skin_rig_pass_state_00519640(&job) == 1u,
        "rig pass terminal returns one");
  check(job.field_010 == 4, "rig pass stops at four");
  check(skin_rig_pass_state_00519640(&job) == 1u, "rig pass stays terminal");
  check(f.trace.start_render == 3u && f.trace.end_render == 3u,
        "rig pass brackets one render per productive call");
}

void test_rig_pass_state_skips_slot_one() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJobState job{};
  job.field_010 = 1;
  skin_rig_pass_state_00519640(&job);
  check(job.field_010 == 3, "rig pass remaps state one onto state two");
  check(f.trace.start_render == 1u, "rig pass state two uses the wide texture");
  check(f.trace.end_render == 1u, "rig pass state two ends the wide texture");
}

void test_rig_pass_state_material_and_raster() {
  ++g_tests;
  Fixture f;
  setup(f, 1);
  OpaqueSkinJobState job{};
  ...
  skin_rig_pass_state_00519640(&job);
  check(f.tex[0].field_004 == kMaterialBase, "rig pass zero base material");
  check(f.tex[0].count_044 == 3, "rig pass zero raises three params");
  check(f.create_count == 2u, "rig pass zero resolves two keys");
  check(f.tex[0].rasters_03c != nullptr, "rig pass binds first raster");
  check(f.tex[0].rasters_040 != nullptr, "rig pass binds second raster");
  check(f.tex[0].count_048 == 2, "rig pass raises raster count to two");
  check(f.trace.manager_prepare == 2u, "rig pass prepares both slots");

  setup(f, 1);
  OpaqueSkinJobState unreachable{};
  unreachable.field_010 = 2;
  skin_rig_pass_state_00519640(&unreachable);
  check(f.tex[0].field_004 == 0u,
        "the state zero and one render blocks are unreachable");
  check(f.tex[0].rasters_040 == nullptr,
        "an unreachable pass binds no second raster");

  setup(f, 1);
  OpaqueSkinJobState rim{};
  rim.field_010 = 2;
  skin_rig_pass_state_00519640(&rim);
  check(f.tex[1].field_004 == kMaterialRim, "rig pass two rim material");
  check(f.tex[1].params_00c.a == 3.0F, "rig pass two uses the painter scale");
  check(f.tex[1].params_00c.d == 1.0F, "rig pass two param alpha is one");
}

void test_rig_pass_state_scale_material() {
  ++g_tests;
  Fixture f;
  setup(f, 1);
  OpaqueSkinJobState scale{};
  scale.field_010 = 3;
  f.scale_value = 4;
  skin_rig_pass_state_00519640(&scale);
  check(f.tex[1].field_004 == kMaterialScale, "rig pass three scale material");
  check(f.tex[1].params_00c.a == 8.0F,
        "rig pass three multiplies by the scale");
  check(f.tex[1].params_00c.b == 8.0F, "rig pass three repeats across rgb");
  check(f.tex[1].params_00c.d == 1.0F, "rig pass three param alpha is one");
  check(f.create_count == 1u, "rig pass three resolves one key");
}

void test_rig_pass_state_scale_fallback() {
  ++g_tests;
  Fixture f;
  setup(f, 1);
  OpaqueSkinJobState fallback{};
  fallback.field_010 = 3;
  f.scale_value = 0;
  skin_rig_pass_state_00519640(&fallback);
  check(f.tex[1].params_00c.a == 2.0F,
        "rig pass three falls back to unit scale");
}

void test_rig_pass_state_pass_limit() {
  ++g_tests;
  Fixture f;
  setup(f, 12);
  OpaqueSkinJobState job{};
  skin_rig_pass_state_00519640(&job);
  check(f.paint_count == 10u, "rig pass honours the ten pass limit");
  check(job.field_014 == 10, "rig pass stops at the limit");
  check(job.field_010 == 0, "rig pass holds the pass while work remains");
}

void test_rig_pass_state_flagged_rig_blocks() {
  ++g_tests;
  Fixture f;
  setup(f, 3);
  f.rigblock[1].flags_008 = 1u;
  OpaqueSkinJobState job{};
  skin_rig_pass_state_00519640(&job);
  check(f.paint_count == 2u, "rig pass skips flagged rig blocks");
  check(job.field_010 == 1, "rig pass still advances the pass");
}

void test_rig_pass_state_can_create_gate() {
  ++g_tests;
  Fixture f;
  setup(f, 1);
  f.can_create_answer = false;
  OpaqueSkinJobState job{};
  skin_rig_pass_state_00519640(&job);
  check(f.paint_count == 0u, "rig pass skips paint when create is refused");
  check(f.trace.manager_prepare == 0u, "rig pass skips prepare when refused");
}

void test_rig_pass_state_desc_selection() {
  ++g_tests;
  Fixture f;
  setup(f, 1);
  OpaqueSkinJobState job{};
  f.target.field_010 = 0x00200000u;
  skin_rig_pass_state_00519640(&job);
  check(f.trace.get_desc == 2u, "rig pass resolves a handle desc per key");
  check(f.trace.get_default_desc == 0u,
        "rig pass skips the default desc for a wide tag");
  check(f.create[0].block[0] == 0x11u, "rig pass forwards the handle desc");
  check(f.create[0].tag == kCreateBlockTags, "rig pass uses the block tag");

  setup(f, 1);
  OpaqueSkinJobState narrow_tag{};
  f.target.field_010 = 0x00100000u;
  skin_rig_pass_state_00519640(&narrow_tag);
  check(f.trace.get_desc == 2u, "rig pass accepts a narrow handle tag");

  setup(f, 1);
  OpaqueSkinJobState no_tag{};
  skin_rig_pass_state_00519640(&no_tag);
  check(f.trace.get_default_desc == 2u,
        "rig pass falls back when the handle tag is unknown");
  check(f.create[0].block[0] == 0x44u, "rig pass forwards the default desc");
}
...
void test_rig_index_pass_null_create() {
  ...
  setup(f, 1);
  OpaqueSkinJobState job{};
  f.acquire_answer = false;
  skin_rig_index_pass_0051a350(&job);
  check(f.trace.manager_create_block == 1u, "rig index pass still creates");
  check(f.trace.acquire_object == 0u, "rig index pass does not acquire");
...
void test_job_setup_stores_arguments() {
  ...
  check(f.trace.collect == 1u, "job setup tails into collect");
  check(job.state_33c == 0u, "job setup leaves the state alone");
}

void test_job_collect_guards() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  f.system.painter_00c = nullptr;
  OpaqueSkinJob job{};
  skin_job_collect_0051ab50(&job, 1u);
  check(f.trace.collect == 0u, "collect stops on a null painter");

  setup(f, 0);
  f.system.mesh_020 = nullptr;
  skin_job_collect_0051ab50(&job, 1u);
  check(f.trace.collect == 0u, "collect stops on a null mesh");

  setup(f, 0);
  skin_job_collect_0051ab50(&job, 1u);
  check(f.trace.collect == 1u, "collect creates when the slot is empty");
  check(f.trace.manager_create_raw == 1u, "collect uses the raw create slot");
  check(f.create[0].tag == kCreateRawKey, "collect passes the raw create key");
  check(f.trace.acquire_slot == 0u, "collect skips acquire when non-empty");
  check(f.trace.rewind == 0u, "collect does not rewind on the create path");
  check(f.trace.release_one == 1u, "collect releases after collecting");
}

void test_job_collect_acquires_and_rewinds() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  f.owner[0] = 0u;
  OpaqueSkinJob job{};
  skin_job_collect_0051ab50(&job, 0u);
  check(f.trace.acquire_slot == 1u, "collect acquires an empty slot");
  check(f.trace.manager_create_raw == 0u,
        "collect skips create when non-empty");
  check(f.trace.rewind == 1u, "collect rewinds after the cursor pass");
  check(f.trace.collect == 2u,
        "collect pushes exactly the two non zero cursor entries");
  check(f.trace.collect_arg[0] ==
            reinterpret_cast<OpaqueRefObject **>(&f.slot_triple[1]),
        "collect takes the first cursor entry from slot value plus four");
  check(f.trace.collect_arg[1] ==
            reinterpret_cast<OpaqueRefObject **>(&f.slot_triple[2]),
        "collect takes the second cursor entry from slot value plus eight");
  check(f.trace.get_key == 0u, "collect stops at the zero argument");
}

void test_job_collect_argument_gate() {
  ++g_tests;
  Fixture f;
  setup(f, 3);
  OpaqueSkinJob job{};
  skin_job_collect_0051ab50(&job, 0u);
  check(f.key_count == 0u, "collect skips rig blocks when the flag is zero");

  setup(f, 3);
  skin_job_collect_0051ab50(&job, 0x01u);
  check(f.key_count == 9u, "collect probes three keys per rig block");
  check(f.key[0].key == kKeySlotA, "collect first key");
  check(f.key[1].key == kKeySlotB, "collect second key");
  check(f.key[2].key == kKeySlotC, "collect third key");
  check(f.key[0].id == f.mesh_id[0], "collect passes the rig block id");
  check(f.create_count == 10u, "collect creates one raw plus nine blocks");
}

void test_job_collect_rig_block_gate() {
  ++g_tests;
  Fixture f;
  setup(f, 3);
  f.rigblock[1].flags_008 = 1u;
  OpaqueSkinJob job{};
  skin_job_collect_0051ab50(&job, 1u);
  check(f.key_count == 6u, "collect skips flagged rig blocks");
}

void test_job_state_walks_states() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  job.array_008 = f.owner;
  job.array_00c = f.owner;

  skin_job_state_step_0051afd0(&job);
  check(job.state_33c == kStateTerminal, "an idle walk terminates in one call");
  check(job.calls_344 == 1u, "state step counts every call");
  check(f.acquire_count == 1u, "an idle walk acquires once");
  check(f.acquire_log[0] == kAcquireSizeStage,
        "state three uses the stage size");
  check(f.trace.job_init_primary == 1u, "state three initialises primary");
  check(f.trace.ref_step == 1u, "state three steps the handle");
  check(f.trace.ref_add == 1u, "state three adds a reference");
}

void test_job_state_scan_stops_the_walk() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  job.array_008 = f.owner;
  job.array_00c = f.owner + 2;
  job.array_008[0] = reinterpret_cast<TargetWord>(&f.element);

  skin_job_state_step_0051afd0(&job);
  check(job.state_33c == 3u, "scan keeps the state on an early return");
  check(job.cursor_340 == 1u, "scan counts the first live element");
  check(f.acquire_count == 0u, "scan returns before acquiring");

  OpaqueSkinJob early{};
  early.array_008 = f.owner;
  early.array_00c = f.owner + 2;
  early.array_008[0] = reinterpret_cast<TargetWord>(&f.element);
  skin_job_state_step_0051afd0(&early);
  check(early.state_33c == 3u, "states zero and one fall straight into two");
  check(early.cursor_340 == 1u, "the scan counts the live element once");
  early.array_008[0] = 0u;
  early.array_008[1] = 0u;
  skin_job_state_step_0051afd0(&early);
  check(early.state_33c == kStateTerminal,
        "state three runs to the end once the scan is empty");
  check(early.calls_344 == 2u, "each call increments the counter once");
}

void test_job_state_scan_gates() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  job.state_33c = 2u;
  job.array_008 = f.owner;
  job.array_00c = f.owner + 1;
  job.array_008[0] = 0u;
  skin_job_state_step_0051afd0(&job);
  check(f.acquire_count == 1u, "scan skips a null element");
  check(job.cursor_340 == 0u, "a null element never counts");

  setup(f, 0);
  OpaqueSkinJob flagged{};
  flagged.state_33c = 2u;
  flagged.array_008 = f.owner;
  flagged.array_00c = f.owner + 1;
  flagged.array_008[0] = reinterpret_cast<TargetWord>(&f.element);
  f.element.opaque_004[0] = 1u;
  skin_job_state_step_0051afd0(&flagged);
  check(f.acquire_count == 1u, "scan skips a flagged element");
  check(flagged.cursor_340 == 0u, "a flagged element never counts");
}

void test_job_state_flag_gates() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  ...
  job.flag_348 = 0u;
  skin_job_state_step_0051afd0(&job);
  check(f.acquire_count == 0u, "gated state acquires nothing");
  check(job.state_33c == kStateTerminal, "gated state runs to the terminal");
  check(job.calls_344 == 1u, "gated state counts one call");
}

void test_job_state_system_flag_gate() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  ...
  job.flag_348 = 1u;
  job.flag_34a = 1u;
  f.system.byte_07c = 0u;
  skin_job_state_step_0051afd0(&job);
  check(job.state_33c == 5u, "state five returns early on the system flag");
  check(f.acquire_count == 0u, "state five acquires nothing");

  setup(f, 0);
  job.state_33c = 5u;
  job.flag_348 = 1u;
  job.flag_34a = 1u;
  OpaqueSkinJob walking{};
  walking.state_33c = 5u;
  walking.flag_348 = 1u;
  walking.flag_34a = 1u;
  f.system.byte_07c = 1u;
  f.target.flag_00c = 1u;
  skin_job_state_step_0051afd0(&walking);
  check(walking.state_33c == kStateTerminal, "state five continues when set");
  check(f.acquire_count == 4u, "state five releases into later states");
  check(f.acquire_log[1] == kAcquireSizeJobWide,
        "state seven acquires while 34e is clear");
  check(f.acquire_log[2] == kAcquireSizeJobWide,
        "state eight acquires after the drop");
}

void test_job_state_acquire_sequence() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  ...
  const TargetWord expected[] = {kAcquireSizeJob,     kAcquireSizeRaw,
                                 kAcquireSizeJobWide, kAcquireSizeJobWide,
                                 kAcquireSizeRaw,     kAcquireSizeMid};
  check(f.acquire_count == sizeof(expected) / sizeof(expected[0]),
        "gated states acquire once each");
  for (std::size_t i = 0; i < sizeof(expected) / sizeof(expected[0]); ++i) {
    check(f.acquire_log[i] == expected[i], "gated state acquire size matches");
  }
  check(f.target.field_014 == kJobField14Eight,
        "states seven and eight set the same count");
}

void test_job_state_flag34e_gate() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob blocked{};
  blocked.state_33c = 7u;
  blocked.flag_348 = 1u;
  blocked.flag_34e = 1u;
  f.target.flag_00c = 1u;
  skin_job_state_step_0051afd0(&blocked);
  check(f.acquire_count == 1u, "state seven skips when 34e is set");
  check(f.acquire_log[0] == kAcquireSizeMid,
        "state ten still acquires with 34e set");
  check(f.target.field_014 == 0u, "a set 34e skips the wide field writes");
}

void test_job_state_build_states() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  job.state_33c = 11u;
  job.field_324 = 0x1234u;
  job.field_320 = 0x5678u;
  job.field_32c = 0x9abcu;
  job.field_328 = 0x2222u;
  job.flag_34d = 0x5au;
  job.flag_34c = 0x77u;
  f.target.flag_00c = 1u;
  f.target2.field_014 = 1u;
  skin_job_state_step_0051afd0(&job);
  check(job.state_33c == kStateTerminal, "build states reach the end");
  check(f.build_count == 2u, "state eleven and thirteen both build");
  check(f.build[0].arg[0] == 0u, "state eleven uses slot zero");
  check(f.build[0].arg[1] == 0x5678u, "state eleven passes 320");
  check(f.build[0].arg[2] == 0x1234u, "state eleven passes 324");
  check(f.build[0].arg[3] == 0x5au, "state eleven passes 34d");
  check(f.build[0].arg[4] == 0x9abcu, "state eleven passes 32c");
  check(f.build[0].arg[5] == 0u, "state eleven passes zero");
  check(f.build[0].arg[6] == 0x77u, "state eleven passes 34c");
  check(f.build[1].arg[0] == 2u, "state thirteen uses slot two");
  check(f.build[1].arg[1] == 0x5678u, "state thirteen passes 320");
  check(f.build[1].arg[2] == 0x2222u, "state thirteen passes 328");
  check(f.build[1].arg[4] == 0xffffffffu, "state thirteen passes minus one");
  check(f.build[1].arg[5] == 0u, "state thirteen passes zero for 34b");
  check(f.build[1].arg[6] == 0x77u, "state thirteen passes 34c");
}

void test_job_state_build_gates() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  ...
  f.target.flag_00c = 1u;
  skin_job_state_step_0051afd0(&job);
  check(f.build_count == 0u, "build states skip when 324 and 328 are zero");
  check(f.acquire_count == 0u, "build states acquire nothing when gated off");
}

void test_job_state_reuse_and_drop() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  job.state_33c = 3u;
  job.handle_330 = &f.target2;
  skin_job_state_step_0051afd0(&job);
  check(f.acquire_count == 0u, "reused handle skips acquire");
  check(f.trace.ref_step == 1u, "reused handle still steps");
  check(job.handle_330 == &f.target2, "reused handle survives a clear flag");

  setup(f, 0);
  job.state_33c = 3u;
  job.handle_330 = &f.target2;
  f.target2.flag_00c = 1u;
  skin_job_state_step_0051afd0(&job);
  check(job.handle_330 == nullptr, "done handle is dropped");
  check(f.trace.ref_release == 1u, "done handle releases once");
}

void test_job_state_terminal() {
  ++g_tests;
  Fixture f;
  setup(f, 0);
  OpaqueSkinJob job{};
  ...
  skin_job_state_step_0051afd0(&job);
  check(job.state_33c == kStateTerminal, "terminal state stays terminal");
  check(f.acquire_count == 0u, "terminal state acquires nothing");

  job.state_33c = 200u;
  skin_job_state_step_0051afd0(&job);
  check(job.state_33c == kStateTerminal, "out of range state terminates");
}

void test_port_table_shape() {
  ++g_tests;
  install_ports();
  check(sizeof(SkinnerWave11Ports) > 0u, "port table is populated");
  check(g_skinner_wave11_ports.get_paint_system_00401080 != nullptr,
        "port table retains the system accessor");
  check(g_skinner_wave11_ports.job_build_0051c470 != nullptr,
        "port table retains the build port");
  check(g_skinner_wave11_ports.paint_region_005296b0 != nullptr,
...
void test_layout_contract() {
  ...
  check(offsetof(OpaqueElementVTable, has_01c) == 0x1cu,
        "element has-block slot offset");
...
int run_skinner_wave11_tests() {
  ...
  test_rig_pass_state_runs_four_passes();
  test_rig_pass_state_skips_slot_one();
  test_rig_pass_state_material_and_raster();
  test_rig_pass_state_scale_material();
  test_rig_pass_state_scale_fallback();
  test_rig_pass_state_pass_limit();
  test_rig_pass_state_flagged_rig_blocks();
  test_rig_pass_state_can_create_gate();
  test_rig_pass_state_desc_selection();
  ...
  test_job_collect_guards();
  test_job_collect_acquires_and_rewinds();
  test_job_collect_argument_gate();
  test_job_collect_rig_block_gate();
  test_job_state_walks_states();
  test_job_state_scan_stops_the_walk();
  test_job_state_scan_gates();
  test_job_state_flag_gates();
  test_job_state_system_flag_gate();
  test_job_state_acquire_sequence();
  test_job_state_flag34e_gate();
  test_job_state_build_states();
  test_job_state_build_gates();
  test_job_state_reuse_and_drop();
  test_job_state_terminal();
...
int main() {
  return openspore::reconstruction::pkg_skinner_wave11::
      run_skinner_wave11_tests();
}
