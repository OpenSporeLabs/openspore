...
void atomic_bump(Word* slot) {
#if defined(_MSC_VER)
  _InterlockedExchangeAdd(reinterpret_cast<volatile long*>(slot), 1L);
#else
  __atomic_fetch_add(slot, 1u, __ATOMIC_SEQ_CST);
#endif
}
...
extern "C" bool W11_THISCALL editor_paint_commit_0043ac40(
...
extern "C" bool W11_CDECL editor_bake_probe_004bf770(OpaqueProbeOwner* owner,
                                                     OpaqueBakeRow* row) {
...
extern "C" bool W11_THISCALL editor_bake_select_004c4a30(
...
extern "C" void W11_THISCALL editor_pair_retain_004c5910(
    OpaquePairRoot* self, OpaqueHandle** first, OpaqueHandle** second) {
  if (first != nullptr) {
    *first = self->first_070;
    atomic_bump(&self->first_070->count_008);
  }
  if (second != nullptr) {
    *second = self->second_074;
    atomic_bump(&self->second_074->count_008);
  }
}

extern "C" Word W11_THISCALL editor_species_resolve_004df550(
    OpaqueLinkRoot* self, const OpaqueSpeciesKey* key) {
  if (key->word_00 == 0u && self->fallback_0a4.word_00 == 0u) {
    return 0u;
  }
  Word status = unresolved_004df6d0(self->anchor_004, key);
  if (status != 0u) {
    return status;
  }
  OpaqueSpeciesKey probe;
  probe.word_00 = key->word_00;
  probe.word_04 = key->word_04;
  probe.word_08 = key->word_08;
  if (probe.word_00 == 0u) {
    probe = self->fallback_0a4;
    status = unresolved_004df6d0(self->anchor_004, &probe);
    if (status != 0u) {
      return status;
    }
  }
  if (!unresolved_004da3a0(&probe)) {
    return 0u;
  }
  status = unresolved_004df6d0(self->anchor_004, &probe);
  if (status != 0u) {
  ...
  void* const handle =
      unresolved_00f473a0(kAcquireProfile, kNameProfile, 0u, 0u, 0u, 0u);
  Word made = 0u;
  if (handle != nullptr) {
    made = unresolved_004d3dd0(handle, &probe, 0u);
  }
  OpaqueLinkCell* const slot = unresolved_004e0560(self->anchor_004, &probe);
  slot->slot_00 = made;
  return unresolved_004df6d0(self->anchor_004, &probe);
}
