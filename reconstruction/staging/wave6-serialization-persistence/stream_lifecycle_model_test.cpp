...
void invoke_initializer(MemoryStream &stream, std::uint32_t ignored_word) {
  std::uintptr_t eax_result = 0;
  std::uint32_t ecx_result = 0;
  std::uint32_t edx_result = 0;
  std::uint32_t flags_result = 0;
  std::uint32_t xmm_result = 0;
  __asm__ volatile(
      "pushl %[ignored]\n\t"
      "movl %[stream], %%ecx\n\t"
      "movl $0x9abcdef0, %%edx\n\t"
      ...
      "pushfl\n\t"
      "popl %[flags_result]\n\t"
      : [eax_result] "=&a"(eax_result), [ecx_result] "=&r"(ecx_result),
        [edx_result] "=&r"(edx_result), [xmm_result] "=m"(xmm_result),
        [flags_result] "=m"(flags_result)
      : [stream] "r"(&stream), [ignored] "r"(ignored_word)
      : "memory");
  check(eax_result == reinterpret_cast<std::uintptr_t>(&stream));
  check(ecx_result == 0);
...
void WAVE6_STREAM_THISCALL release_parent(IStream *parent) {
  ...
  check(observed_child->active_parent == nullptr);
...
void test_stream_child_lifecycle_delete_bit() {
  ...
  check(stream_child_close_and_maybe_delete_0093b610(&child, 1U) == &child);
