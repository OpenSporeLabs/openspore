...
void* read_vtable(const void* object) {
  void* value;
...
ViewerItemSlot08 item_slot08(const OpaqueViewerObject* object) {
  void* const* table = reinterpret_cast<void* const*>(read_vtable(object));
...
ViewerSlot34 viewer_slot34(OpaqueAdvancedItemViewer* viewer) {
  void* const* table = reinterpret_cast<void* const*>(read_vtable(viewer));
  return reinterpret_cast<ViewerSlot34>(table[13]);
}

GraphicsSlot04 graphics_slot04(OpaqueGraphicsRoot* root) {
  void* const* table = reinterpret_cast<void* const*>(read_vtable(root));
  return reinterpret_cast<GraphicsSlot04>(table[1]);
}

GraphicsSlot34 graphics_slot34(OpaqueGraphicsContext* context) {
  void* const* table = reinterpret_cast<void* const*>(read_vtable(context));
  return reinterpret_cast<GraphicsSlot34>(table[13]);
}

AppSlot14 app_slot14(OpaqueAppSystem* app) {
  void* const* table = reinterpret_cast<void* const*>(read_vtable(app));
  return reinterpret_cast<AppSlot14>(table[5]);
}

}

float g_palette_width_multiplier = 0.05F;
float g_palette_height_multiplier = 0.0F;

extern "C" OpaquePaletteMain* PKG_PALETTE_THISCALL
pkg_palette_005c5ee0(OpaquePaletteMain* self, std::uint32_t category_id) {
...
extern "C" OpaquePalettePage* PKG_PALETTE_THISCALL
pkg_palette_005c8bc0(OpaquePalettePage* self, std::uint32_t key) {
  if (key == 0xee3f516eU || key == 0x2f009dd0U || key == 0x72deed2bU) {
    return self;
  }
  return nullptr;
}
...
pkg_palette_005f4750(OpaqueAdvancedItemViewer* self) {
  ...
  OpaqueSwatchState* swatch_state = unresolved_0113ae10(swatch_manager);
  if (swatch_state != nullptr && self->field16b != 0U) {
    ...
    if (item->field14 != nullptr && item->field31 != 0U) {
      object_slot28(item->field14)(
          item->field14, self->field180,
          reinterpret_cast<OpaqueResourceKey*>(
              reinterpret_cast<std::uint8_t*>(self) + 0x4cU),
          item);
    } else {
      OpaqueViewerObject* allocated =
          unresolved_00f473a0(100U, 0x13eb430U, 0U, 0U, 0U, 0U);
      if (allocated != nullptr) {
        constructed = unresolved_0059f030(
            allocated, read_u32(self->field180, 0x0cU),
            read_u32(self->field180, 0x10U), read_u32(self->field180, 0x14U),
            item->field2c, 0x14880158U, 0U, 1U);
    ...
    unresolved_008283a0(
        ...
        (rect->field08 - rect->field00) * g_palette_width_multiplier,
        (rect->field0c - rect->field04) * g_palette_height_multiplier);
  }

  OpaqueRenderTarget* render_target =
      unresolved_0067cad0(viewer_slot34(self)(self, 1U, 1U));
  unresolved_0080d710(render_target, 1U, 1U);
  if (self->field194 != nullptr) {
    OpaqueRenderTarget* secondary_render_target =
        unresolved_0067cad0(static_cast<OpaqueRenderTarget*>(self->field194));
    unresolved_0080d710(secondary_render_target, 1U, 1U);
  }

  OpaqueCommandLine command_line{};
  command_line.field00 =
