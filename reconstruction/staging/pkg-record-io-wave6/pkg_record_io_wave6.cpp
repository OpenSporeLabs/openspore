...
using ReadDataSlot = void(PKG_RECORD_IO_THISCALL*)(
    OpaqueIStream*, void*, std::uint32_t);
using SeekSlot = std::uint32_t(PKG_RECORD_IO_THISCALL*)(
    OpaqueIStream*, std::int32_t, std::uint32_t);
using ReadSlot = std::int32_t(PKG_RECORD_IO_THISCALL*)(
    OpaqueIStream*, std::uint32_t, void*);

constexpr std::size_t kRecordReadFallback = 0x24;
constexpr std::size_t kRecordReadPrimary = 0x48;
constexpr std::size_t kRecordWriteValid = 0x1c;
constexpr std::size_t kRecordWriteEmbeddedStream = 0x2c;
constexpr std::size_t kRecordWritePosition = 0x60;
constexpr std::size_t kRecordWriteLimit = 0x64;
constexpr std::size_t kRecordWriteState = 0x68;
...
void record_io_set_record_write_ports(RecordWritePorts new_ports) {
  ports.prepare = new_ports.prepare == nullptr ? default_prepare
                                               : new_ports.prepare;
...
void PKG_RECORD_IO_THISCALL record_read_data_008dc820(
  ...
  OpaqueIStream* primary = reinterpret_cast<OpaqueIStream*>(
      load<std::uint32_t>(record->bytes, kRecordReadPrimary));
  OpaqueIStream* fallback = reinterpret_cast<OpaqueIStream*>(
      load<std::uint32_t>(record->bytes, kRecordReadFallback));
...
bool PKG_RECORD_IO_THISCALL record_write_seek_008dcab0(
  ...
  if (load<std::uint32_t>(record->bytes, kRecordWriteValid) == 0u) {
    return false;
  }
  ports.prepare(record);
  if (load<std::uint8_t>(record->bytes, kRecordWriteState) != 0u) {
    OpaqueIStream* stream = reinterpret_cast<OpaqueIStream*>(
        record->bytes + kRecordWriteEmbeddedStream);
    const SeekSlot seek = load_slot<SeekSlot>(stream, 0x28u);
    return seek(stream, amount, operation) != 0u;
  }

  std::uint32_t position =
      load<std::uint32_t>(record->bytes, kRecordWritePosition);
  const std::uint32_t limit =
      load<std::uint32_t>(record->bytes, kRecordWriteLimit);
  ...
  store<std::uint32_t>(record->bytes, kRecordWritePosition, position);
  return true;
}
...
std::int32_t PKG_RECORD_IO_THISCALL record_write_read_008dcb70(
  ...
  if (available != 0u && !ports.read(record, data, position, available)) {
    available = 0u;
  }
  position += available;
  store<std::uint32_t>(record->bytes, kRecordWritePosition, position);
