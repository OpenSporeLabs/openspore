...
sources="$package_dir/app_services_wave11.cpp $package_dir/app_services_wave11_model_test.cpp"
headers="$package_dir/app_services_wave11.hpp"
...
clang-format --style="$style" --dry-run -Werror \
  "$headers" \
  "$package_dir/app_services_wave11.cpp" \
  "$package_dir/app_services_wave11_model_test.cpp"
echo "format: clean"
...
for spec in o0:'-O0' o2:'-O2' o0n:'-O0 -DNDEBUG' o2n:'-O2 -DNDEBUG'; do
  ...
  clang++ -m32 -std=c++17 $flags $warnings -I "$package_dir" \
    -c "$package_dir/app_services_wave11.cpp" -o "$work/$tag.o"
...
CLOSURE = "pkg_app_services_wave11"
ALLOWED = re.compile(
