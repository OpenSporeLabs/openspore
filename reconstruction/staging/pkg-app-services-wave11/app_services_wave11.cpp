...
inline OpaqueBool low_byte(OpaqueWord value) {
  return (value & 0xffu) != 0u;
}
...
extern "C" OpaqueBool PKG_W11_THISCALL service_005fc330(OpaqueService *self,
  ...
  if (found != 0u) {
    const OpaqueWord *entry = reinterpret_cast<const OpaqueWord *>(
  ...
  release_handle(pending, 0x04u);
  {
    OpaqueGate *gate_object = reinterpret_cast<OpaqueGate *>(
        static_cast<std::uintptr_t>(gate_handle));
  ...
  if (node_owner != 0u) {
    OpaqueOwner *record = reinterpret_cast<OpaqueOwner *>(
        static_cast<std::uintptr_t>(node_owner));
...
extern "C" void PKG_W11_THISCALL service_0060ee90(OpaqueOwner60ee90 *self,
  ...
  if ((pair[0] & pair[1]) == 0xffffffffu) {
    ports.drop_00551af0(reinterpret_cast<OpaqueWord *>(
