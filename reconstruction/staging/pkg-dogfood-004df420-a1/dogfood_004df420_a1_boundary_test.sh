#!/bin/sh
# Boundary test for PKG-DOGFOOD-004DF420-A1.
#
# Two jobs, in order:
...
#      NDEBUG;
#   2. disassemble the emitted body and adjudicate it against what the original
#      28 bytes at 0x004df420 actually prove: one receiver in ECX, one outgoing
#      dword at receiver + 0xa4, exactly one call and only to the proven port, a
#      bare return, no branch, no indirect dispatch, no absolute image address,
#      and no write to the return register after the call.
#
# Every expectation below cites the instruction it comes from. Nothing here is
# a style preference.
set -eu

package_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
work_dir=/tmp/opencode/pkg-dogfood-004df420-a1
...
if command -v clang-format >/dev/null 2>&1; then
  for source in dogfood_004df420_a1.hpp \
                dogfood_004df420_a1.cpp \
                dogfood_004df420_a1_model_test.cpp; do
...
for config in $binaries; do
  ...
  clang++ -m32 -std=c++17 $flags -Wall -Wextra -Wpedantic -Werror \
    -I "$package_dir" "$package_dir/dogfood_004df420_a1.cpp" \
    "$package_dir/dogfood_004df420_a1_model_test.cpp" -o "$binary"
  "$binary"
...
BODY = r"^[0-9a-f]+ <%s>:\n(?P<body>.*?)(?=\n[0-9a-f]+ <|\Z)"
TARGET = "dogfood_species_key_resolve_fun_004df420"
...
# links the recording stub under this name, so the VA is not the original one and
...
for text in listing:
    ...
    for mnemonic, operands in instructions:
        # The original body has no branch of any kind: twelve instructions, one
        # block. A conditional would mean the model added a decision the original
        # never made.
