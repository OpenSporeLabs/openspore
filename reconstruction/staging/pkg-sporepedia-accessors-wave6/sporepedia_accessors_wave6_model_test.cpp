...
Pkg16AssetMetadata* expected_time_metadata = nullptr;
...
extern "C" __thiscall void pkg16_time_helper_00550cf0(Pkg16AssetMetadata* value,
  ...
  assert(value == expected_time_metadata);
  ++time_helper_calls;
...
void reset() {
  ...
  expected_time_metadata = nullptr;
...
void test_time_accessor() {
  reset();
  expected_time_metadata = &metadata;
  store_asset_word(
      asset, 0x1c,
      static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(&metadata)));
  ...
  sporepedia_get_time_created_00641860(&asset, &destination);
  ...
  store_asset_word(
      asset, 0x1c,
      static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(&metadata)));
  sporepedia_get_time_created_00641860(&asset, nullptr);
  assert(time_helper_calls == 1);
  assert(observed_time_destination == nullptr);
}
...
[[noreturn]] void run_invalid(InvalidKind kind) {
  ...
  switch (kind) {
    ...
    case InvalidKind::time:
      sporepedia_get_time_created_00641860(self, &destination);
      break;
  }
  _exit(result == 0 ? 2 : 3);
}
