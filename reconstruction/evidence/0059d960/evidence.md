# Evidence 0x0059d960

- Evidence state: `LIVE`
- Live requested: `True`
- Content SHA-256: `2c847f3226531bd0b654f99698be6ab62cd1645a077ba8e1a9d2f270bd826064`

## abi

- Availability: `available`
- Evidence state: `DERIVED`
- Provenance: `reconstruction/knowledge/index.json, GhidraMCP REST /get_function_by_address + /analyze_function_complete @ http://127.0.0.1:8089, GhidraMCP /disassemble_function`

```json
{
  "abi": {
    "architecture": "x86-32",
    "calling_convention": "__fastcall",
    "hidden_this": true,
    "hidden_this_register": "ECX",
    "receiver": true,
    "receiver_register": "ECX",
    "ret_form": "RET",
    "return_register": "XMM0",
    "return_semantics": "float_or_x87_in_XMM0",
    "stack_cleanup_bytes": 0,
    "stack_cleanup_owner": "caller",
    "termination": "RET"
  },
  "abstained_because": [],
  "cleanup": {
    "bytes": 0,
    "confidence": "INFERRED",
    "corroboration": "not_available",
    "evidence": "ret with no immediate, no stack reads",
    "side": "caller"
  },
  "completeness": "CORE_RESOLVED",
  "conflicts": [],
  "content_sha256": "e651e629bf172f02727364fab1df4296232b80cb7c1de9a316ae49a8ba667378",
  "conventions": {
    "ambiguities": [],
    "calling_convention": "__fastcall",
    "candidate_conventions": [
      "__fastcall"
    ],
    "confidence": "INFERRED",
    "corroboration": "not_available"
  },
  "cross_validation": {
    "agreement": false,
    "ghidra": "no_information",
    "ghidra_calling_convention": null,
    "ghidra_parameter_count": 0,
    "persisted": "no_information",
    "persisted_calling_convention": null
  },
  "dispatch": {
    "call_offsets": [],
    "indirect_calls": 0,
    "vtable_shaped_loads": 0
  },
  "inferences": [
    {
      "based_on": [
        "obs-0008"
      ],
      "claim": "the caller cleans up the stack: a bare RET is compatible with caller cleanup and, for a zero-parameter __stdcall, with zero bytes of callee cleanup",
      "confidence": "INFERRED",
      "id": "C5",
      "value": {
        "bytes": 0,
        "side": "caller"
      }
    },
    {
      "based_on": [
        "obs-0001",
        "obs-0002",
        "obs-0004"
      ],
      "claim": "ECX carries a receiver and is dereferenced before any definite write to it",
      "confidence": "SUPPORTED",
      "id": "R1",
      "value": {
        "offsets": [
          0,
          4,
          12,
          16,
          20,
          24,
          28,
          32,
          36,
          40,
          44
        ],
        "register": "ECX",
        "written_through": 13
      }
    },
    {
      "based_on": [
        "obs-0005"
      ],
      "claim": "EDX is a memory base before any definite write to it",
      "confidence": "OBSERVED",
      "id": "C8-E",
      "value": {
        "register": "EDX"
      }
    },
    {
      "based_on": [
        "obs-0001",
        "obs-0002",
        "obs-0004",
        "obs-0005"
      ],
      "claim": "calling convention is __fastcall: ECX and EDX are both volatile, so reading the incoming EDX before any write to it can only be a compiler-guaranteed register argument",
      "confidence": "INFERRED",
      "id": "C8",
      "value": "__fastcall"
    },
    {
      "based_on": [
        "obs-0008"
      ],
      "claim": "entry slot 0 is not written through a pointer",
      "confidence": "APPROXIMATION",
      "id": "S2",
      "value": {
        "present": false
      }
    },
    {
      "based_on": [
        "obs-0008"
      ],
      "claim": "the return value is carried in XMM0: an x87 or SSE instruction appears in the body",
      "confidence": "APPROXIMATION",
      "id": "RT1",
      "value": "XMM0"
    }
  ],
  "observations": [
    {
      "at": "0x0059d960",
      "count": 10,
      "first_use": 0,
      "first_write_index": 3,
      "id": "obs-0001",
      "index": 0,
      "kind": "REG_READ",
      "raw": "MOV EAX,ECX",
      "reg": "ECX"
    },
    {
      "at": "0x0059d960",
      "definite": true,
      "id": "obs-0002",
      "index": 0,
      "kind": "REG_WRITE",
      "raw": "MOV EAX,ECX",
      "reg": "EAX",
      "write_kind": "reg"
    },
    {
      "at": "0x0059d962",
      "count": 14,
      "first_use": 1,
      "first_write_index": 0,
      "id": "obs-0003",
      "index": 1,
      "kind": "REG_READ",
      "raw": "MOV dword ptr [EAX],0x13eb918",
      "reg": "EAX"
    },
    {
      "at": "0x0059d96f",
      "definite": true,
      "id": "obs-0004",
      "index": 3,
      "kind": "REG_WRITE",
      "raw": "XOR ECX,ECX",
      "reg": "ECX",
      "write_kind": "zero"
    },
    {
      "at": "0x0059d971",
      "count": 2,
      "first_use": 4,
      "first_write_index": null,
      "id": "obs-0005",
      "index": 4,
      "kind": "REG_READ",
      "raw": "LEA EDX,[EAX + 0x8]",
      "reg": "EDX"
    },
    {
      "at": "0x0059d976",
      "count": 3,
      "first_use": 6,
      "first_write_index": 6,
      "id": "obs-0006",
      "index": 6,
      "kind": "REG_READ",
      "raw": "XORPS XMM0,XMM0",
      "reg": "XMM0"
    },
    {
      "at": "0x0059d976",
      "definite": true,
      "id": "obs-0007",
      "index": 6,
      "kind": "REG_WRITE",
      "raw": "XORPS XMM0,XMM0",
      "reg": "XMM0",
      "write_kind": "unknown"
    },
    {
      "at": "0x0059d9b3",
      "form": "RET",
      "id": "obs-0008",
      "imm": null,
      "index": 20,
      "kind": "RET",
      "raw": "RET"
    }
  ],
  "parse": {
    "declared_count": 21,
    "degraded": false,
    "esp_unresolved": false,
    "flow_complete": true,
    "frame": {
      "and_esp": null,
      "ebp_is_general_register": false,
      "fp": false,
      "lea_esp": null,
      "mov_ebp_esp": false,
      "mov_ebp_esp_at": null,
      "push_ebp": false,
      "push_ebp_at": null,
      "sub": null
    },
    "layout": "json_instruction_list",
    "local_extent": 0,
    "unparsed": 0
  },
  "receiver": {
    "bounds_only": true,
    "confidence": "SUPPORTED",
    "distinct_offsets": 11,
    "max_offset": 44,
    "offsets": [
      0,
      4,
      12,
      16,
      20,
      24,
      28,
      32,
      36,
      40,
      44
    ],
    "present": true,
    "register": "ECX",
    "shape": "R-ALIAS",
    "written_through": 13
  },
  "return": {
    "aggregate_evidence": {
      "bulk_write": false
    },
    "confidence": "APPROXIMATION",
    "register": "XMM0",
    "register_class": "float_or_x87",
    "type": n
[TRUNCATED]
```

## callees_dependencies

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## callers_dependencies

- Availability: `available`
- Evidence state: `DERIVED`
- Provenance: `reconstruction/knowledge/index.json`

```json
[
  {
    "name": null,
    "reconstructed": false,
    "va": "0x00573970"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x00574110"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x0057e480"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x00587270"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x0058a350"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x0058a5a0"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x00591690"
  },
  {
    "name": "Editors::cEditor::HandleMessage",
    "reconstructed": false,
    "va": "0x00591fa0"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x005ad430"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x005b1e30"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x005b4fa0"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x005b94b0"
  },
  {
    "name": null,
    "reconstructed": false,
    "va": "0x005bc630"
  }
]
```

## contradictions

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## decompilation

- Availability: `available`
- Evidence state: `LIVE`
- Provenance: `GhidraMCP REST /decompile_function @ http://127.0.0.1:8089`

```json
"\nvoid __fastcall cEditorAnimEvent__ctor(undefined4 *param_1)\n\n{\n  *param_1 = &PTR_FUN_013eb918;\n  param_1[1] = &PTR_LAB_013f65b4;\n  LOCK();\n  param_1[2] = 0;\n  UNLOCK();\n  param_1[8] = 0;\n  *param_1 = &PTR_LAB_013f65d4;\n  param_1[1] = &PTR_LAB_013f65c4;\n  param_1[3] = 0;\n  param_1[4] = 0;\n  param_1[5] = 0;\n  param_1[6] = 0;\n  *(undefined1 *)(param_1 + 7) = 0;\n  *(undefined1 *)(param_1 + 9) = 0;\n  param_1[10] = 0xffffffff;\n  param_1[0xb] = 0x3f800000;\n  return;\n}\n\n"
```

## disassembly

- Availability: `available`
- Evidence state: `LIVE`
- Provenance: `GhidraMCP /disassemble_function`

```json
{
  "count": 21,
  "instructions": [
    {
      "address": "0059d960",
      "instruction": "MOV EAX,ECX"
    },
    {
      "address": "0059d962",
      "instruction": "MOV dword ptr [EAX],0x13eb918"
    },
    {
      "address": "0059d968",
      "instruction": "MOV dword ptr [EAX + 0x4],0x13f65b4"
    },
    {
      "address": "0059d96f",
      "instruction": "XOR ECX,ECX"
    },
    {
      "address": "0059d971",
      "instruction": "LEA EDX,[EAX + 0x8]"
    },
    {
      "address": "0059d974",
      "instruction": "XCHG dword ptr [EDX],ECX"
    },
    {
      "address": "0059d976",
      "instruction": "XORPS XMM0,XMM0"
    },
    {
      "address": "0059d979",
      "instruction": "XOR ECX,ECX"
    },
    {
      "address": "0059d97b",
      "instruction": "MOVSS dword ptr [EAX + 0x20],XMM0"
    },
    {
      "address": "0059d980",
      "instruction": "MOVSS XMM0,dword ptr [0x01485720]"
    },
    {
      "address": "0059d988",
      "instruction": "MOV dword ptr [EAX],0x13f65d4"
    },
    {
      "address": "0059d98e",
      "instruction": "MOV dword ptr [EAX + 0x4],0x13f65c4"
    },
    {
      "address": "0059d995",
      "instruction": "MOV dword ptr [EAX + 0xc],ECX"
    },
    {
      "address": "0059d998",
      "instruction": "MOV dword ptr [EAX + 0x10],ECX"
    },
    {
      "address": "0059d99b",
      "instruction": "MOV dword ptr [EAX + 0x14],ECX"
    },
    {
      "address": "0059d99e",
      "instruction": "MOV dword ptr [EAX + 0x18],ECX"
    },
    {
      "address": "0059d9a1",
      "instruction": "MOV byte ptr [EAX + 0x1c],CL"
    },
    {
      "address": "0059d9a4",
      "instruction": "MOV byte ptr [EAX + 0x24],CL"
    },
    {
      "address": "0059d9a7",
      "instruction": "MOV dword ptr [EAX + 0x28],0xffffffff"
    },
    {
      "address": "0059d9ae",
      "instruction": "MOVSS dword ptr [EAX + 0x2c],XMM0"
    },
    {
      "address": "0059d9b3",
      "instruction": "RET"
    }
  ]
}
```

## external_callees

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## function_identity

- Availability: `available`
- Evidence state: `DERIVED`
- Provenance: `reconstruction/knowledge/index.json`

```json
{
  "original_bytes": 7347,
  "preview": "{\n  \"abi\": {},\n  \"analogues\": [\n    {\n      \"match_basis\": [\n        \"same_subsystem\"\n      ],\n      \"package\": \"PKG-01-SHARED-STATE-ROOTS\",\n      \"score\": 6,\n      \"symbol\": \"FUN_00ff3f00\",\n      \"va\": \"0x00ff3f00\"\n    }\n  ],\n  \"audit_evidence_boundary\": null,\n  \"audit_findings\": [],\n  \"audit_status\": null,\n  \"blocked\": false,\n  \"blockers\": [],\n  \"body_status\": null,\n  \"class_type\": null,\n  \"cluster\": \"gameglobal-misc\",\n  \"confidence\": null,\n  \"dependencies\": {\n    \"callees\": [],\n    \"callees_truncated\": false,\n    \"callers\": [\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x00573970\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x00574110\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x0057e480\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x00587270\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x0058a350\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x0058a5a0\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x00591690\"\n      },\n      {\n        \"name\": \"Editors::cEditor::HandleMessage\",\n        \"reconstructed\": false,\n        \"va\": \"0x00591fa0\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x005ad430\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x005b1e30\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x005b4fa0\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x005b94b0\"\n      },\n      {\n        \"name\": null,\n        \"reconstructed\": false,\n        \"va\": \"0x005bc630\"\n      }\n    ],\n    \"callers_truncated\": false,\n    \"data_reference_count\": 0,\n    \"edges\": [\n      {\n        \"callsite\": \"0x005739b0\",\n        \"direction\": \"in\",\n        \"other\": \"0x00573970\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x00574196\",\n        \"direction\": \"in\",\n        \"other\": \"0x00574110\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x0057e5d7\",\n        \"direction\": \"in\",\n        \"other\": \"0x0057e480\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x0057e703\",\n        \"direction\": \"in\",\n        \"other\": \"0x0057e480\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005878fc\",\n        \"direction\": \"in\",\n        \"other\": \"0x00587270\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x00587998\",\n        \"direction\": \"in\",\n        \"other\": \"0x00587270\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x0058a522\",\n        \"direction\": \"in\",\n        \"other\": \"0x0058a350\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x0058a81e\",\n        \"direction\": \"in\",\n        \"other\": \"0x0058a5a0\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x00591c96\",\n        \"direction\": \"in\",\n        \"other\": \"0x00591690\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x00592a31\",\n        \"direction\": \"in\",\n        \"other\": \"0x00591fa0\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005ad471\",\n        \"direction\": \"in\",\n        \"other\": \"0x005ad430\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005ad4c5\",\n        \"direction\": \"in\",\n        \"other\": \"0x005ad430\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005ad531\",\n        \"direction\": \"in\",\n        \"other\": \"0x005ad430\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b2011\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b1e30\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b2065\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b1e30\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b20ce\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b1e30\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b5031\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b4fa0\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b94f6\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b94b0\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b956d\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b94b0\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005b95be\",\n        \"direction\": \"in\",\n        \"other\": \"0x005b94b0\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005bc671\",\n        \"direction\": \"in\",\n        \"other\": \"0x005bc630\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        \"callsite\": \"0x005bc6c5\",\n        \"direction\": \"in\",\n        \"other\": \"0x005bc630\",\n        \"reference_type\": \"direct-call\"\n      },\n      {\n        
[TRUNCATED]
```

## ghidra_function

- Availability: `available`
- Evidence state: `LIVE`
- Provenance: `GhidraMCP REST /get_function_by_address + /analyze_function_complete @ http://127.0.0.1:8089`

```json
{
  "binary_available": true,
  "binary_sha256": "25d42a7a5c4d438fb155233230f57d29e2849bfdff5c889a5d0847f0469d914e",
  "body_end": "0059d9b3",
  "body_span_bytes": 84,
  "body_start": "0059d960",
  "callees": [],
  "callers": [
    "FUN_0058a350",
    "FUN_00591690",
    "FUN_005bc630",
    "FUN_005b94b0",
    "Editors::cEditor::PostEventToActors",
    "Editors::cEditor::SetActiveMode",
    "Editors::cEditor::SetCreatureToNeutralPose",
    "FUN_0057e480",
    "FUN_005b1e30",
    "Editors::cEditor::HandleMessage",
    "FUN_005b4fa0",
    "FUN_005ad430",
    "Editors::cEditor::Undo"
  ],
  "classification": "leaf",
  "dispatch": null,
  "entry_point": "0059d960",
  "evidence_note": "decompiler output = evidence, not truth; no MSVC RTTI in this binary",
  "ghidra_calling_convention": null,
  "ghidra_calling_convention_role": "cross-validation-only",
  "ghidra_calling_convention_signal": "no_information",
  "ghidra_has_calling_convention": false,
  "image_base": "0x400000",
  "locals": [
    {
      "name": "param_1",
      "storage": "register:00000004:4",
      "type": "undefined4 *"
    }
  ],
  "locals_count": 1,
  "mode": "live",
  "name": "cEditorAnimEvent__ctor",
  "namespace": null,
  "namespace_source": null,
  "parameter_count": 0,
  "parameters": [],
  "program": "SporeApp.exe",
  "provenance": "GhidraMCP REST /get_function_by_address + /analyze_function_complete @ http://127.0.0.1:8089",
  "return_type": "undefined",
  "return_type_resolved": false,
  "rva": "0x19d960",
  "sdk_name": null,
  "sdk_type": null,
  "signature": "undefined cEditorAnimEvent__ctor(void)",
  "size_bytes": 84,
  "status": "ok",
  "subsystem": null,
  "tool": "ghidra_function",
  "va": "0x0059d960",
  "vtables": {
    "referenced_by_vtables": [],
    "sdk_associations": [],
    "vtable_at": []
  },
  "xref_count": 34,
  "xrefs": [
    {
      "from": "00574196"
    },
    {
      "from": "005878fc"
    },
    {
      "from": "00587998"
    },
    {
      "from": "005739b0"
    },
    {
      "from": "0058a81e"
    },
    {
      "from": "005ad471"
    },
    {
      "from": "005ad4c5"
    },
    {
      "from": "005ad531"
    },
    {
      "from": "005b2011"
    },
    {
      "from": "005b2065"
    },
    {
      "from": "005b20ce"
    },
    {
      "from": "005b5031"
    },
    {
      "from": "005b94f6"
    },
    {
      "from": "005b956d"
    },
    {
      "from": "005b95be"
    },
    {
      "from": "005bc671"
    },
    {
      "from": "005bc6c5"
    },
    {
      "from": "005bc731"
    },
    {
      "from": "0058a522"
    },
    {
      "from": "0057e5d7"
    },
    {
      "from": "0057e703"
    },
    {
      "from": "00592a31"
    },
    {
      "from": "00591c96"
    },
    {
      "from": "005ab098"
    },
    {
      "from": "005ab127"
    },
    {
      "from": "005acb1f"
    },
    {
      "from": "005adbd5"
    },
    {
      "from": "005adc26"
    },
    {
      "from": "005adc8c"
    },
    {
      "from": "005b41ae"
    },
    {
      "from": "005b4222"
    },
    {
      "from": "005b4273"
    },
    {
      "from": "005b7969"
    },
    {
      "from": "005bd6fc"
    }
  ]
}
```

## globals

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## reconstruction

- Availability: `available`
- Evidence state: `PERSISTED`
- Provenance: `reconstruction/knowledge/index.json`

```json
{
  "files": [],
  "handoffs": [],
  "metadata": []
}
```

## runtime

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## runtime_metadata

- Availability: `available`
- Evidence state: `PERSISTED`
- Provenance: `reconstruction/knowledge/index.json`

```json
{
  "blocking_reason": null,
  "gates": [],
  "validated": 0
}
```

## semantic_hypotheses

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `knowledgegraph/research/semantic-decomp.json`

## status

- Availability: `available`
- Evidence state: `DERIVED`
- Provenance: `reconstruction/knowledge/index.json`

```json
{
  "runtime_gated": false,
  "runtime_validated": 0,
  "status": "candidate"
}
```

## types

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## vtables

- Availability: `unavailable`
- Evidence state: `MISSING`
- Provenance: `reconstruction/knowledge/index.json`

## Conflicts

```json
[]
```
