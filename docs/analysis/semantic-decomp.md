...
No source file, Ghidra program, canonical knowledge-graph database, `SPORE/` content, package document, or historical artifact was modified. This consistency pass changed only the eight campaign worker JSONs, the canonical campaign JSON, and the two campaign reports.
...
|---:|---|---|---|---|---:|---|
| 1 | `0x00b21340` | `cGameNounManager::GetData`; typed noun vector, lower-bound selection, dirty rebuild | Strong | `0.97`; mechanics/purpose high | 254 direct callers; 254 semantic downstream | `worker-00-roots.json:45-364` |
...
| 5 | `0x00e5c780` | Generic unsigned ordered-map exact-find primitive | Bounded | `0.96`; mechanics `0.99` | 239 unique direct caller functions | `worker-01-gameglobal.json:24-151` |
...
| Ordered maps | Map end `+0x04`, root `+0x0c`, node key `+0x10`, payload `+0x14`; `cGameNounManager +0x98`; `cStarManager +0x150` | Live body at `0x00e5c780` rejects a greater successor, making it exact-find; this conflicts with lower-bound wording elsewhere. `worker-01-gameglobal.json:81-96` |
...
|---|---|---|
| Noun projection | `0x00b21340` transitions absent/end to dirty record, dirty to rebuilt record, clean to borrowed return; callbacks are create/clear/add/filter. `0x01021300` transitions ID `-1`, identity hit, lower-bound/miss refresh, and null lookup with explicit reference order. | Root slot publication/equality/teardown, callback reentrancy, and whether a real manager ever supplies a successor. The exact-find contradiction must be resolved before successor-dependent claims are implemented. |
...
|---|---|---|
| Root/map spine | Typed map adapter, dual alternate/canonical root ports, noun projection dirty-state, current-empire cache | Do not collapse alternate and canonical root slots. Do not implement lower-bound successor behavior until the `0x00e5c780` exact-find contradiction is adjudicated. |
...
|---|---|---|---|
| `PKG-01-SHARED-STATE-ROOTS` / `PKG-11-SIM-CORE` | Static-ready for opaque borrowed manager/root adapters; current-empire cache is strong; alternate root slots remain runtime-gated. | Publisher/equality/teardown matrix for `0x0167eae0`, `0x0167eae4`, and canonical siblings; exact-vs-lower-bound map adjudication. | Define separate root ports and borrowed ownership. Test exact hit, missing successor, end hit, null, and dual-slot phase changes. Order: map primitive → root publication/lifetime → manager projection → high-fan-in consumers. |
...
Recommended implementation ordering is therefore: **(1)** resolve the map/root contradiction and dual-slot lifecycle; **(2)** type Cell/Space state and ownership boundaries; **(3)** type editor command/mode/history and persistence; **(4)** type palette/UI service adapters; **(5)** keep Sporepedia local and resolve only the two unknown event families; **(6)** run only narrowly approved runtime separators for the remaining lifetime and timing questions.

## Contradictions and adjudications

1. **Exact find versus lower bound.** `0x00e5c780`'s final candidate equality check and adjacent exact getter support exact find (`worker-01-gameglobal.json:81-103,125-143`). `worker-00-roots.json:89-100,278-284` describes the helper and noun path as lower-bound and permits a successor. The live target body should be treated as the stronger current evidence, but `0x00b21340` and `0x01021300` successor claims must be rechecked against it before implementation.
...
|---:|---|---|---|
| 1 | `0x00e5c780` → `0x00b21340` → `0x00b3d300`/`0x01021300` map/root spine | Resolves the exact-vs-lower contradiction, noun projection, current-empire cache, and the largest root fan-in. | Static caller policy check for exact/in-between keys, then an approved dual-slot publication/equality/teardown trace; never silently preserve successor behavior. |
