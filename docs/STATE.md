...
S4 is complete with fallback; S5 is gated and remains the next milestone. The execution plan for cell-stage completion is `docs/analysis/campaigns/cell-stage-campaign.md` (32 targets / 5 waves). **Wave 1 (CS-01–CS-04 static decode unblockers) is COMPLETE and does not need S5** — the decoders are all in-tree C++ with differential tests. **Wave 2 (CS-05…CS-12 cell resource decodes, all build on CS-03) is next.** Two gates remain, in priority order:
