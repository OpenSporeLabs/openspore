...
bool meshFromGmdl(const GmdlModel &model, uint32_t meshIndex, Mesh &out,
  ...
  if (ib.bytes.size() < static_cast<size_t>(ib.indexCount) * 2) {
  ...
  if (vb.bytes.size() < static_cast<size_t>(vb.vertexCount) * stride) {
  ...
  for (uint32_t i = 0; i < vb.vertexCount; ++i) {
    ...
    out.positions.push_back(
        {readLEFloat(p), readLEFloat(p + 4), readLEFloat(p + 8)});
    if (nrmEl != nullptr) {
    ...
    if (uvEl != nullptr) {
      ...
      out.uvs.push_back({readLEFloat(t), readLEFloat(t + 4)});
    }
  }
