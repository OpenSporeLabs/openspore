...
//
// Clean-room re-implementation of the NAMED / meaningful fields of the
// singleton `Simulator::Cell::sCellGame` (VERIFIED address 0x16b3c04). The
// large fixed_vector and cObjectPool backing stores are represented as capacity
...
// A non-main pool (query / rollover / linked pools) is kept as a capacity
// handle — only the main object pool (mCells) is the real cObjectPool (CS-16).
...
enum class CellGameLifecycleStatus { success, unsupported, failure };

struct CellGameConfig {
  uint32_t poolCapacity = 4096;
  float timeScale = 1.0F;
};

struct CellGameLifecycleResult {
  CellGameLifecycleStatus status = CellGameLifecycleStatus::success;

  explicit operator bool() const {
    return status == CellGameLifecycleStatus::success;
  }
};
...
// mpCellQueryBackground, mCells, mShowCallMateButtonCountdown,
// mpSerializableData); anonymous SDK fields keep their `field_NNN` / `flag_NNN`
// names with the SDK offset embedded in the name.
struct CellGame {
  // Pool + query (SDK offsets 28 / 16636 / 16640 / 56..196).
  cObjectPool<cCellObjectData> mCells;  // main object pool, capacity 4096
  CellMovementLifecycle mMovementState;
  uint32_t mConfiguredCapacity = 0;
  ...
  CellGameLifecycleResult initialize();
  CellGameLifecycleResult initialize(const CellGameConfig& config);
  CellGameLifecycleResult reset();
  CellGameLifecycleResult tick(float deltaSeconds);
  ...
  uint32_t createCellObject(const CellResourceSpec& resource,
  ...
  cCellObjectData* objectAt(uint32_t index) const { return mCells.at(index); }
  cCellObjectData* lookup(uint32_t index) const { return mCells.lookup(index); }
  bool isAllocated(uint32_t index) const { return mCells.isAllocatedAt(index); }

  CellMovementResult
  applyCellMovement(uint32_t targetIndex, const CellMovementInput& input,
                    const OrientationResolver& orientationResolver);
  CellMovementResult
  routeCellMovement(uint32_t targetIndex,
