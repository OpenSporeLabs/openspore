...
void testSelfInclusive() {
  ...
  check(r.size() == 1 && contains(r, 7), "query: a cell at the query point is in range");
}
...
void testTouchingBorder() {
  ...
  check(contains(r, 1) && !contains(r, 2), "query: touching cell in, beyond cell out");
}
...
void testRadiusExpands() {
  ...
  check(contains(r, 3) && !contains(r, 4), "query: radius extends the reach (1.2 in, 1.9 out)");
}

void testCluster() {
  // Fixture world: a 3-cell cluster; only the two nearest cells reach the query.
  CellQuery q;
  q.reserve(16);
  const float p[3] = {0.0F, 0.0F, 0.0F};
