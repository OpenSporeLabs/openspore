...
#if defined(_MSC_VER)
#define PKG_RECORD_IO_CDECL __cdecl
#define PKG_RECORD_IO_THISCALL __thiscall
#else
#define PKG_RECORD_IO_CDECL __attribute__((cdecl))
...
struct RecordWritePorts {
  using Prepare = void(PKG_RECORD_IO_CDECL*)(OpaqueRecordWrite*);
  using Read = bool(PKG_RECORD_IO_CDECL*)(OpaqueRecordWrite*, void*,
...
static_assert(sizeof(OpaqueRecordRead) == 0x70, "PFRecordRead opaque size");
static_assert(sizeof(OpaqueRecordWrite) == 0x27c, "PFRecordWrite opaque size");
static_assert(sizeof(OpaqueFixedMemoryStream) == 0x18,
              "FixedMemoryStream opaque size");
static_assert(sizeof(OpaqueMemoryStream) == 0x24, "MemoryStream opaque size");
...
bool PKG_RECORD_IO_THISCALL
fixed_memory_stream_seek_0093b950(OpaqueFixedMemoryStream* stream,
                                  std::int32_t amount, std::uint32_t operation);

void record_io_set_record_write_ports(RecordWritePorts ports);

}

#undef PKG_RECORD_IO_CDECL
