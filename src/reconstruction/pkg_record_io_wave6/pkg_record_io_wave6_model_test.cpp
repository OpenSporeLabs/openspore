...
#if defined(_MSC_VER)
#define PKG_RECORD_IO_TEST_CDECL __cdecl
#define PKG_RECORD_IO_TEST_THISCALL __thiscall
#else
#define PKG_RECORD_IO_TEST_CDECL __attribute__((cdecl))
...
using openspore::reconstruction::pkg_record_io_wave6::record_read_data_008dc820;
using openspore::reconstruction::pkg_record_io_wave6::
...
OpaqueRecordWrite* observed_record;
void* observed_data;
...
void set_slot(OpaqueIStreamVtable& vtable, std::size_t offset,
  ...
  std::memcpy(reinterpret_cast<std::uint8_t*>(&vtable.slots[0]) + offset, &word,
              sizeof(word));
}

void store_vtable(void* base, std::size_t offset, OpaqueIStreamVtable* vtable) {
...
void PKG_RECORD_IO_TEST_CDECL prepare_record(OpaqueRecordWrite* record) {
  events.push_back("prepare");
  observed_record = record;
  observed_stream = reinterpret_cast<OpaqueIStream*>(record);
}

bool PKG_RECORD_IO_TEST_CDECL read_record(OpaqueRecordWrite* record, void* data,
...
std::int32_t PKG_RECORD_IO_TEST_THISCALL memory_read_slot(OpaqueIStream* stream,
                                                          std::uint32_t size,
                                                          void* data) {
...
void reset() {
  ...
  record_io_set_record_write_ports(
      RecordWritePorts{prepare_record, read_record});
}
...
void test_record_read_dispatch() {
  ...
  store_vtable(record.bytes, 0x04, &fallback_vtable);
  store_vtable(record.bytes, 0x28, &primary_vtable);
  ...
  assert(observed_stream ==
         reinterpret_cast<OpaqueIStream*>(record.bytes + 0x28));
  ...
  assert(events[1] == "read-data");
  assert(observed_stream ==
         reinterpret_cast<OpaqueIStream*>(record.bytes + 0x04));
...
void test_record_write_read_local() {
  ...
  assert(record_write_read_008dcb70(&record,
