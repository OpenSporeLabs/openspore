...
work_dir=/tmp/opencode/pkg-utfwin-safe-wave11
...
if command -v clang-format >/dev/null 2>&1; then
  for source in utfwin_safe_wave11.hpp utfwin_safe_wave11.cpp \
                utfwin_safe_wave11_model_test.cpp; do
...
for config in $binaries; do
  ...
  clang++ -m32 -std=c++17 $flags -Wall -Wextra -Wpedantic -Werror \
    -I "$package_dir" "$package_dir/utfwin_safe_wave11.cpp" \
    "$package_dir/utfwin_safe_wave11_model_test.cpp" -o "$binary"
  "$binary"
...
# return width, and the number of unresolved contracts the body must still
# reach. 0x005c00e0 is deliberately absent: it is an SDK-boundary surface that
# this batch did not promote, so the promoted package must not define it.
TARGETS = {
    "utfwin_safe_wave11_005ac9f0": (0x005AC9F0, {0x00}, "dword", 1),
}
WIDTH_MOVES = {
...
CONTRACTS = (
    "utfwin_safe_wave11_base_construct_005b0f80",
    "utfwin_safe_wave11_base_destroy_005c0070",
    "utfwin_safe_wave11_release_00f47380",
)
FORBIDDEN = (
    "utfwin_safe_wave11_005c00e0",
    "utfwin_safe_wave11_base_destroy_005c0070",
    "utfwin_safe_wave11_release_00f47380",
)
...
for path in sys.argv[1:]:
    ...
    for absent in FORBIDDEN:
        if absent in text:
            raise SystemExit(f"{path}: unpromoted surface {absent} is still present")
    ...
    first = body_of(path, "utfwin_safe_wave11_005ac9f0")
    if "utfwin_safe_wave11_base_construct_005b0f80" not in first:
        raise SystemExit(f"{path}: 005ac9f0 lost its base constructor call")
    print(f"{path}: ret width and stack cleanup match the target for 0x005ac9f0")
PY
