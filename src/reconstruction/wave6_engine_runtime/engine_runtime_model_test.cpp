...
std::vector<std::size_t> appended_lengths;
std::vector<OpaqueWave6WideString*> appended_destinations;
std::vector<const std::uint16_t*> appended_sources;

void TEST_THISCALL append_wide_string_span(OpaqueWave6WideString* destination,
                                           const std::uint16_t* source,
                                           const std::uint16_t* end) {
  const std::string name = destination == nullptr ? "null" : "destination";
  wide_string_trace.emplace_back("append-span:" + name);
  appended_lengths.push_back(static_cast<std::size_t>(end - source));
  appended_destinations.push_back(destination);
  appended_sources.push_back(source);
}

void TEST_THISCALL append_wide_string_unit(OpaqueWave6WideString* destination,
                                           std::uint16_t unit) {
  const std::string name = destination == nullptr ? "null" : "destination";
  wide_string_trace.emplace_back("append-unit:" + name);
...
void test_plugin_initialization_order() {
  ...
  ports.append_00423650 = &append_wide_string_span;
  ports.append_004f6510 = &append_wide_string_unit;
  ...
  std::array<std::uint16_t, 3> second_command_line = {
  ...
  appended_lengths.clear();
  appended_destinations.clear();
  appended_sources.clear();

  openspore::reconstruction::wave6_engine_runtime::
      app_system_initialize_plugins_007e93d0(&registration, command_line.data(),
                                             second_command_line.data());

  check(wide_string_trace ==
            std::vector<std::string>(
                {"append-span:destination", "append-unit:destination",
                 "append-span:destination", "append-unit:destination"}),
        "plugin initialization appends spans and units in two pairs");
  check(appended_lengths == std::vector<std::size_t>({3U, 2U}),
        "each span append receives its source terminator");
  check(appended_units == std::vector<std::uint16_t>({0x005cU, 0x005cU}),
        "both copies append one backslash unit");
  check(appended_destinations ==
            std::vector<OpaqueWave6WideString*>{
                &registration.first_command_line_copy_138,
                &registration.second_command_line_copy_148},
        "span appends target the first and second observed windows");
  check(appended_sources ==
            std::vector<const std::uint16_t*>{command_line.data(),
                                              second_command_line.data()},
        "span appends receive the two command-line stack pointers");
  ...
  std::array<std::uint16_t, 1> empty_second_command_line = {0U};
  wide_string_trace.clear();
  appended_units.clear();
  appended_lengths.clear();
  appended_destinations.clear();
  ...
  openspore::reconstruction::wave6_engine_runtime::
      app_system_initialize_plugins_007e93d0(&registration,
                                             ...
                                             empty_second_command_line.data());
  check(appended_lengths == std::vector<std::size_t>({0U, 0U}),
        "empty command lines produce two zero-length span appends");
  check(appended_units == std::vector<std::uint16_t>({0x005cU, 0x005cU}),
        "empty command lines still append two separators");
