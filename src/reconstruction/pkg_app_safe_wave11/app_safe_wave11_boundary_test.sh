...
work_dir=/tmp/opencode/pkg-app-safe-wave11
...
if command -v clang-format >/dev/null 2>&1; then
  for source in app_safe_wave11.hpp app_safe_wave11.cpp app_safe_wave11_model_test.cpp; do
...
for config in $binaries; do
  ...
  clang++ -m32 -std=c++17 $flags -Wall -Wextra -Wpedantic -Werror \
    -I "$package_dir" "$package_dir/app_safe_wave11.cpp" \
    "$package_dir/app_safe_wave11_model_test.cpp" -o "$binary"
  "$binary"
...
# 0x004af260 is the anchor of four open conflict ledgers and is not promoted
# here, so the promoted package must contain neither its body nor the seven
# stage ports that only it called.
EXCLUDED = (
    "model_stage_serialize_004af260",
    "OpaqueStageModel",
    "OpaqueStageEntry",
    "OpaqueStageRecord",
    "record_bit",
    "lookup_004b0010",
    "classify_004bb860",
    "record_grow_004afdd0",
    "record_reserve_004afd00",
    "weight_reserve_004afc80",
    "key_reserve_004cd3c0",
    "element_gate_0044f220",
)
...
for path in sys.argv[1:]:
    ...
    print(
        ...
        " 0x00428060, 0x00447150, 0x004786e0, 0x004b62a0 and 0x004bc540"
    )
PY

echo "boundary: all four configurations built, ran and matched"
echo "boundary: 0x004af260 and its seven stage ports are absent"
echo "boundary: pkg-app-safe-wave11 validated"
