...
work=/tmp/opencode/pkg-palette-safe-wave11
style='{BasedOnStyle: Google, FixNamespaceComments: false}'
sources="$package_dir/palette_safe_wave11.cpp $package_dir/palette_safe_wave11_model_test.cpp"
headers="$package_dir/palette_safe_wave11.hpp"
...
clang-format --style="$style" --dry-run -Werror \
  "$headers" \
  "$package_dir/palette_safe_wave11.cpp" \
  "$package_dir/palette_safe_wave11_model_test.cpp"
echo "format: clean"
...
for spec in o0:'-O0' o2:'-O2' o0n:'-O0 -DNDEBUG' o2n:'-O2 -DNDEBUG'; do
  ...
  clang++ -m32 -std=c++17 $flags $warnings -I "$package_dir" \
    -c "$package_dir/palette_safe_wave11.cpp" -o "$work/$tag.o"
...
for tag in o0 o2 o0n o2n; do
  ...
  if [ -z "$checks" ] || [ "$checks" -lt 248 ]; then
    echo "model: $tag ran only ${checks:-0} checks, expected at least 248" >&2
    exit 1
  fi
done
...
LINK = "palette_safe_wave11_resolve_link_tail_005c2e00"
RELEASE = "palette_safe_wave11_release_page_hold_005c7320"
WALK = "palette_safe_wave11_walk_has_leaf_005c7c10"
FILL = "palette_safe_wave11_fill_node_array_005c7ff0"
LOAD = "palette_safe_wave11_load_page_state_005c85d0"

# 0x005ca9f0 is excluded from this tranche, so it is asserted absent rather
# than measured. Its reconstruction mixes the 0x005c2ec0 argument order and the
# model_shift_slot argument order, and whether the two blend results are
# distinct is still open.
ENTRY = (LINK, RELEASE, WALK, FILL, LOAD)

EXCLUDED = (
    "palette_safe_wave11_place_page_nodes_005ca9f0",
    "unresolved_005ca9c0",
    "unresolved_005c2ec0",
    "g_float_013eb95c",
    "g_float_014854c4",
...
for tag in ("o0", "o2", "o0n", "o2n"):
    ...
    if "mul" not in text:
        fail(f"{tag}: the signed divide is gone")
    for absent in EXCLUDED:
        if absent in text:
            fail(f"{tag}: unpromoted surface {absent} is still present")
...
print("boundary: the crossed port set equals the 25 declared ports")
print("boundary: 005c7c10 keeps its arithmetic shift and its signed divide")
...
for port, total in sorted(touched.items()):
    print(f"boundary:   {port} crossed by {total} of 5 entries")
for symbol in EXCLUDED:
    print(f"boundary: {symbol} is absent from every configuration")
PY

echo "boundary: pkg-palette-safe-wave11 validated"
