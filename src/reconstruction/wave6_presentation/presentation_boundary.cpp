...
renderware_mesh_set_index_buffer_011f9710(Mesh* receiver) {
  ...
  if (g_d3d_boundary_ports.prepare_mesh(receiver) == 0) {
    return;
  }
  ...
  std::uint32_t stream_limit = g_stream_limit_015d0934;
  if (stream < stream_limit) {
    for (;;) {
      const std::int32_t stream_result = g_d3d_boundary_ports.set_stream_source(
          g_device_016f89d0, stream, nullptr, 0, 0);
      stream_limit = g_stream_limit_015d0934;
      if (stream_result < 0) {
        return;
      }
      const std::size_t cache_index = static_cast<std::size_t>(stream) * 3U;
