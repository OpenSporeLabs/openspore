...
OpaqueMapEntry* lower_bound_entries(const OpaqueMap& map, TargetWord key) {
  ...
  while (count > 0) {
    ...
    if (candidate->field_00 < key) {
      begin =
          reinterpret_cast<std::uintptr_t>(candidate) + sizeof(OpaqueMapEntry);
...
void copy_property(OpaqueProperty& destination, const OpaqueProperty& source) {
  ...
  if ((source.field_10 & 0x0030u) == 0) {
    ...
    std::memcpy(&pointer_word, source.field_00_0f.data(), sizeof(pointer_word));
...
void set_property_word(OpaqueProperty& destination, TargetWord value,
  ...
  if ((destination.field_10 & 0x0002u) == 0 || destination.field_12 == type) {
...
void set_property_float(OpaqueProperty& destination, float value) {
  ...
  if ((destination.field_10 & 0x0002u) == 0 || destination.field_12 == 0x0du) {
    std::memcpy(destination.field_00_0f.data(), &value, sizeof(value));
...
void PKG20_WAVE6_THISCALL opaque_list_set_property_006a30c0(
  ...
  if (type == 1u && (source->field_10 & 0x0010u) == 0) {
    set_fast_bool(opaque_fast_list, property_id, load_property_bool(source));
    return;
  }
