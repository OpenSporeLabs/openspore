...
OpaqueProperty* PKG20_WAVE6_TEST_THISCALL
resolve_property(OpaquePropertyService*, TargetWord property_id) {
...
void PKG20_WAVE6_TEST_THISCALL set_slow_property(OpaqueList*,
...
bool PKG20_WAVE6_TEST_THISCALL parent_get_property(OpaqueBaseObject*,
                                                   TargetWord property_id,
...
OpaqueProperty* PKG20_WAVE6_TEST_THISCALL
parent_get_property_object(OpaqueBaseObject*, TargetWord property_id) {
  return property_id == 79u ? &parent_property : nullptr;
}

void PKG20_WAVE6_TEST_THISCALL insert_base_property(OpaqueList*,
                                                    TargetWord property_id,
                                                    OpaqueProperty*) {
  ++base_insert_calls;
...
void set_map_entry(OpaqueMapEntry& entry, TargetWord key, std::uint16_t type,
                   std::uint16_t flags, std::uint32_t value) {
  entry.field_00 = key;
...
void test_get_property_object_and_default() {
  ...
  check(opaque_list_get_property_object_006a2800(&list, 1u) == &list.field_40);
  ...
  list.field_40 = OpaqueProperty{};
  std::memcpy(&list_values[1], properties[1].field_00_0f.data(), sizeof(float));
  check(opaque_list_get_property_object_006a2800(&list, 1u) == &list.field_40);
