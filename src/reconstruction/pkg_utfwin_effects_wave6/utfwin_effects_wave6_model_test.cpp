...
void PKG_UTFWIN_THISCALL color_slot_5c(OpaqueColorTarget* target,
                                       Opaque value) {
...
void test_0096fec0_factory_and_dispatch() {
  Opaque instance{};
  const FactorySlot slot =
      function_from<FactorySlot>(function_word(utfwin_0096fec0));
  reset();
...
void test_0097e440_factory_and_dispatch() {
  Opaque instance{};
  const FactorySlot slot =
      function_from<FactorySlot>(function_word(utfwin_0097e440));
  reset();
...
void test_constant_return_slots() {
  const ConstantSlot glide =
      function_from<ConstantSlot>(function_word(utfwin_0096ffc0));
  const ConstantSlot simple =
      function_from<ConstantSlot>(function_word(utfwin_0097e890));
  const ConstantSlot modulate =
      function_from<ConstantSlot>(function_word(utfwin_00980200));
...
void test_adjusted_this_slots() {
  const AdjustSlot inflate =
      function_from<AdjustSlot>(function_word(utfwin_0097e550));
  const AdjustSlot perspective =
      function_from<AdjustSlot>(function_word(utfwin_00980470));
  reset();
...
void test_0097e990_branches_and_mutation_order() {
  const RectTransformSlot slot =
      function_from<RectTransformSlot>(function_word(utfwin_0097e990));
  ...
  destination = {20.0f, 30.0f, 50.0f, 70.0f};
  assert(slot(&layout, &source, &destination));
  assert_rect(destination, 14.0f, 21.0f, 44.0f, 61.0f);
...
void test_0097ea50_field_copies() {
  const RectStoreSlot slot =
      function_from<RectStoreSlot>(function_word(utfwin_0097ea50));
  OpaqueLayout layout{};
...
void test_00980120_blend_then_virtual_dispatch() {
  ...
  const ModulateSlot slot =
      function_from<ModulateSlot>(function_word(utfwin_00980120));
  reset();
