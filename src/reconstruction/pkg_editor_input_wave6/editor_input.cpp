...
bool call_mode_mouse_down(OpaqueEditor* editor, TargetWord button,
...
bool call_selection_move(OpaqueEditor* editor, float mouse_x, float mouse_y,
  ...
  const bool result =
      target->vtable->mouse_move_1c(target, mouse_x, mouse_y, mouse_state);
...
bool PKG_EDITOR_INPUT_THISCALL editor_input_005737d0(OpaqueEditor* editor,
  ...
  const bool near_button =
      button == 0x3e9 || button == 0x3ea ||
      (button == 0x3e8 && selection_target(editor) == nullptr);
  if (near_button) {
    const float x_distance =
        std::fabs(mouse_x - static_cast<float>(load_i32(editor, 0xc0)));
    if (x_distance > 3.0F) {
      store<std::uint8_t>(editor, 0x2b4, 0);
      return false;
    }
    const float y_distance =
        std::fabs(mouse_y - static_cast<float>(load_i32(editor, 0xc4)));
    if (y_distance <= 3.0F) {
      return false;
    }
    store<std::uint8_t>(editor, 0x2b4, 0);
...
bool PKG_EDITOR_INPUT_THISCALL editor_input_00585890(OpaqueEditor* editor,
  ...
  g_input_hooks->game_key_up(
      static_cast<void*>(reinterpret_cast<std::uint8_t*>(editor) + 0xf8), key,
      modifiers);
  ...
  g_input_hooks->state_refresh(editor);
  if (modifiers >= 0x10 && modifiers <= 0x12) {
...
bool PKG_EDITOR_INPUT_THISCALL editor_input_00588570(OpaqueEditor* editor,
  ...
  const bool guarded_click =
      button == 0x3e9 || button == 0x3ea ||
      (button == 0x3e8 && load_u32(editor, 0xcc) == 0 &&
       load_u32(editor, 0xe4) == 0 && load_u8(editor, 0xe8) == 0 &&
       load_u32(editor, 0x31c) != 2 && load_u32(editor, 0xd0) == 0);
  if (guarded_click) {
    store<std::uint8_t>(editor, 0x2b4, 1);
    return false;
  }
  if (!g_input_hooks->graphics_available() || load_u32(editor, 0x84) == 0) {
    return false;
  }
...
bool PKG_EDITOR_INPUT_THISCALL editor_input_0058ac10(OpaqueEditor* editor,
  ...
  const bool primary_clear = !g_input_hooks->key_gate_primary();
  const bool secondary_clear = !g_input_hooks->key_gate_secondary();
  ...
  if (key == 0x42 && (key & 7u) == 0) {
    g_input_hooks->key_external_path(editor, key);
  }

  if (key == 0x46 && (load_u32(editor, 0x30) & 2u) != 0 && (key & 7u) != 6) {
    auto* object = load<OpaqueEditorObject*>(editor, 0xd4);
    if (object != nullptr &&
        (load<TargetWord>(object, 0xdcc) & (1u << 25)) != 0 &&
        (load<TargetWord>(object, 0xdc8) & (1u << 5)) == 0) {
      g_input_hooks->key_external_path(editor, key);
      return false;
    }
  }

  if (key == 0x48 && (key & 7u) == 2 && primary_clear && secondary_clear) {
    g_input_hooks->key_external_path(editor, key);
    return false;
  }

  if (key == 0x4e && primary_clear && secondary_clear) {
    if ((key & 7u) == 0 && load_u32(editor, 0x31c) != 2) {
      g_input_hooks->key_external_path(editor, key);
      return false;
    }
    if ((key & 7u) == 2) {
...
bool PKG_EDITOR_INPUT_THISCALL editor_input_0058b650(OpaqueEditor* editor,
  ...
  if (button == 0x3e9 || (button == 0x3e8 && selection == nullptr)) {
    store<std::uint8_t>(editor, 0x2b3, 1);
  ...
  if ((button == 0x3e8 || button == 0x3ea) && selection != nullptr) {
    ...
    const bool status = selection->vtable->status_34(selection);
    if (status && load_u32(editor, 0xd0) != 0) {
